...
export default function Hero() {
    ...
    // Cambiar entre misión y visión cada 5 segundos
    useEffect(() => {
        ...
        }, 8500); // Cambia cada 5 segundos
        return () => clearInterval(interval); // Limpia el intervalo al desmontar el componente
    }, []);

    return (
        <div className="w-full flex items-center justify-center pt-8 md:pt-1">
            <div className="flex flex-col md:flex-row items-center justify-between w-10/12">
                ...
                <div className="relative md:w-1/2 overflow-hidden mt-8 md:mt-0 md:ml-12">
                    <div
                        className={`transition-transform duration-500 ease-in-out ${slide ? 'translate-x-0' : 'translate-x-full'} w-full flex`}
                    >
                        {/* Misión */}
                        <div className={`w-full ${!isMission ? 'hidden' : ''}`}>
